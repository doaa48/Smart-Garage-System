...
{
    public class DAL_Statstics
    {
        DBConnect db = new DBConnect();

        #region Select from DB

         public DataTable SelectTroubles()
        {
        ...
        public BLL_Reservation GetCustomerDataRelatedToTrouble(int TroubleID)
        {
            BLL_Reservation reservation = null; // Initialize as null
            string Sql = @"SELECT Reservation_ConsumPhone,Reservation_VehicleNumber,Reservation_EnterDateTime,Reservation_ExitDateTime
            FROM Reservation WHERE Reservation_TroubleID=@TroubleID";
            ...
            try
            {
                ...
                {
                    // Add the parameter to prevent SQL injection
                    cmd.Parameters.AddWithValue("@TroubleID", TroubleID);
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        ...
                        if (reader.Read())
                        {
                            reservation = new BLL_Reservation
                            {
                                Reservation_ConsumPhone = reader["Reservation_ConsumPhone"].ToString(),
                                Reservation_VehicleNumber = reader["Reservation_VehicleNumber"].ToString(),
                                Reservation_EnterDateTime = Convert.ToDateTime(reader["Reservation_EnterDateTime"]),
                                Reservation_ExitDateTime = Convert.ToDateTime(reader["Reservation_ExitDateTime"]),
            ...
            return  reservation ; // Initialize as null
    ...
    (@Reservation_ConsumName, @Reservation_ConsumPhone, @Reservation_VehicleNumber, @Reservation_EnterDateTime, @Reservation_ExitDateTime, @Reservation_Payment, @Reservation_VehicleType, @Reservation_MonthlyState, @Reservation_StartMonth, @Reservation_MonthsNumber)";
        ...
        public int GetDailyReservations()
        {
            ...
            string sql = @"SELECT COUNT(Reservation_ID) FROM Reservation WHERE CAST(Reservation_EnterDateTime AS DATE) = CAST(GETDATE() AS DATE)";
            ...
            try
            {
                using (SqlCommand cmd = new SqlCommand(sql, db.GetConn()))
                {
                    ...
                    if (result != null)
                    {
                        DailyReservations = Convert.ToInt32(result);
                    }
                }
            }
            catch (Exception ex)
        ...
        public int GetAllReservations()
        {
            ...
            string sql = @"SELECT COUNT(Reservation_ID) 
                         FROM Reservation 
                         WHERE YEAR(Reservation_EnterDateTime) = YEAR(GETDATE())";
            ...
            try
            {
                using (SqlCommand cmd = new SqlCommand(sql, db.GetConn()))
                {
                    ...
                    if (result != null)
                    {
                        AllReservations = Convert.ToInt32(result);
                    }
                }
            }
            catch (Exception ex)
